...
$InventoryScriptBlock = {
    ...
    # -------------------------------------------------------------------
    # LDAP (389) connectivity check - simple Pass/Fail for the
    # management-facing "Connectivity Health" tile. Tests that the LDAP
    # port is open and that an anonymous-context bind to RootDSE succeeds.
    # -------------------------------------------------------------------
    try {
        $ldapPortTest = Test-NetConnection -ComputerName "localhost" -Port 389 -WarningAction SilentlyContinue
        $ldapPortOpen = [bool]$ldapPortTest.TcpTestSucceeded

        $ldapBindOk = $false
        if ($ldapPortOpen) {
            try {
                $rootDse = [ADSI]"LDAP://localhost/RootDSE"
                $null = $rootDse.Get("defaultNamingContext")
                $ldapBindOk = $true
            }
            catch {
                $ldapBindOk = $false
        ...
        $result.LDAP = [ordered]@{
            Port          = 389
            PortOpen      = $ldapPortOpen
            BindSucceeded = $ldapBindOk
            Status        = if ($ldapPortOpen -and $ldapBindOk) { "Pass" } else { "Fail" }
        }
    }
    catch {
        $result.LDAP = [ordered]@{
            Port = 389; PortOpen = $false; BindSucceeded = $false; Status = "Fail"
        }
    }

    # -------------------------------------------------------------------
    # Additional AD port connectivity checks (Kerberos, DNS, Global
    # Catalog, ADWS) - simple Pass/Fail for the management-facing
    # "Connectivity Health Check" tile.
    # -------------------------------------------------------------------
    $adPortChecks = @(
    ...
    foreach ($check in $adPortChecks) {
        try {
            $portTest = Test-NetConnection -ComputerName "localhost" -Port $check.Port -WarningAction SilentlyContinue
            $portOpen = [bool]$portTest.TcpTestSucceeded

            $result[$check.Name] = [ordered]@{
                Port     = $check.Port
                PortOpen = $portOpen
                Status   = if ($portOpen) { "Pass" } else { "Fail" }
            }
        }
        catch {
            $result[$check.Name] = [ordered]@{
                Port = $check.Port; PortOpen = $false; Status = "Fail"
            }
        }
    }
    ...
    # --- Installed server roles (AD DS best-practice check) ---
    # Lists the top-level Windows Server roles installed on this DC. Best
    # practice is for domain controllers to run only AD DS and DNS (plus
    # their management tools); any other installed role increases the
    # attack surface and is flagged as an "extra" role in the report.
    try {
        $coreRoleNames = @('AD-Domain-Services', 'DNS')
        ...
        $result.SecurityPosture.InstalledRoles = [ordered]@{
            Core  = @($installedRoles | Where-Object { $coreRoleNames -contains $_.Name } | ForEach-Object { $_.DisplayName })
            Extra = @($installedRoles | Where-Object { $coreRoleNames -notcontains $_.Name } | ForEach-Object { $_.DisplayName })
        }
    }
    catch {
        $result.SecurityPosture.InstalledRoles = [ordered]@{ Core = @(); Extra = @(); Error = "$_" }
    }

    # --- Windows Firewall profile status ---
    # Collects the Enabled state of all three firewall profiles (Domain,
    # Private, Public). On a DC the Domain profile must be enabled; the
    # others should also be enabled as defence-in-depth. A disabled profile
    # is flagged as a security risk in the report.
    try {
        $fwProfiles = @(
        ...
        $result.SecurityPosture.Firewall = [ordered]@{
            Profiles = @(
        ...
        $result.SecurityPosture.Firewall = [ordered]@{ Profiles = @(); Error = "$_" }
    }
    ...
    # --- MaxTokenSize (Kerberos token buffer) ---
    # KB2732618: default 12000 is too small for users in 150+ groups.
    # Recommended value is 65535. A missing or low value causes silent
    # Kerberos authentication failures for highly-privileged accounts.
    try {
        $kerberosParams = Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\Kerberos\Parameters" -ErrorAction Stop
        $result.SecurityPosture.MaxTokenSize = if ($null -ne $kerberosParams.MaxTokenSize) { [int]$kerberosParams.MaxTokenSize } else { $null }
    }
    catch {
        $result.SecurityPosture.MaxTokenSize = $null
    }

    # --- Strict Replication Consistency ---
    # Must be 1 on ALL DCs. When 0, AD accepts replication from DCs that
    # have been offline long enough to accumulate lingering objects, which
    # then silently propagate to the rest of the forest.
    try {
        $ntdsReplParams = Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\NTDS\Parameters" -ErrorAction Stop
        $srcValue = $ntdsReplParams.'Strict Replication Consistency'
        $result.SecurityPosture.StrictReplicationConsistency = if ($null -ne $srcValue) { [int]$srcValue } else { $null }
    }
    catch {
        $result.SecurityPosture.StrictReplicationConsistency = $null
    }
    ...
    # --- DFSR MaxOfflineTimeInDays ---
    # The maximum number of days a DC can be offline before DFSR marks its
    # replicated folders as stale and requires a non-authoritative restore.
    # Default is 60 days. Values below 30 are flagged as a warning.
    try {
        $dfsrConfig = Get-WmiObject -Namespace "root\MicrosoftDFS" `
                                    -Class "DfsrMachineConfig" `
                                    -ErrorAction Stop
        $result.DeepInsights.MaxOfflineTimeInDays = [int]$dfsrConfig.MaxOfflineTimeInDays
    }
    catch {
        $result.DeepInsights.MaxOfflineTimeInDays = $null
    }
    ...
    # --- Lingering Object events (1388, 1988) and USN Rollback (2095) ---
    # These are targeted checks separate from the generic error sweep above:
    #   1388 - Lingering object found; replication allowed (loose mode)
    #   1988 - Lingering object blocked (strict replication consistency = 1)
    #   2095 - USN rollback detected; split-brain replication risk
    try {
        $criticalReplEvents = @(Get-WinEvent -FilterHashtable @{ LogName = 'Directory Service'; Id = @(1388, 1988, 2095) } -MaxEvents 20 -ErrorAction Stop)

        $result.DeepInsights.LingeringObjectEvents = @(
            $criticalReplEvents | Where-Object { $_.Id -in 1388, 1988 } | Select-Object -First 5 | ForEach-Object {
                $firstLine = ($_.Message -split "`r?`n" | Select-Object -First 1)
                $truncated = if ($firstLine.Length -gt 200) { $firstLine.Substring(0, 200) + '...' } else { $firstLine }
                [ordered]@{
                    Time    = $_.TimeCreated.ToString("dd-MMM-yyyy HH:mm:ss")
                    EventID = $_.Id
                    Message = $truncated
        ...
        $result.DeepInsights.USNRollbackEvents = @(
            $criticalReplEvents | Where-Object { $_.Id -eq 2095 } | Select-Object -First 5 | ForEach-Object {
                $firstLine = ($_.Message -split "`r?`n" | Select-Object -First 1)
                $truncated = if ($firstLine.Length -gt 200) { $firstLine.Substring(0, 200) + '...' } else { $firstLine }
                [ordered]@{
                    Time    = $_.TimeCreated.ToString("dd-MMM-yyyy HH:mm:ss")
        ...
        # No events found = healthy (log NoEvents for clean detection in dashboard)
        $result.DeepInsights.LingeringObjectEvents = @()
        $result.DeepInsights.USNRollbackEvents     = @()
    }
        ...
        return @(
            $rows | Where-Object { $_."Source DSA" } | ForEach-Object {
                [ordered]@{
                    ...
                    Transport           = $_."Transport Type"
                    LastSuccess         = $_."Last Success Time"
                    LastFailureTime     = $_."Last Failure Time"
...
function Get-AllDCInventory {
    ...
    foreach ($dc in $ComputerName) {
        ...
        if ($match) {
            foreach ($section in 'Hardware','System','Performance','Disks','Services','TimeSync','LDAPS','LDAP','Kerberos','DNS','GlobalCatalog','GlobalCatalogSSL','ADWS','SMB','Network','EthernetSettings','SecurityPosture','DeepInsights') {
